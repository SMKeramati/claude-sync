...
- **MCP servers.** The `mcpServers` block of every `claude_desktop_config.json` is reconciled. Add a server in one profile: it appears in every profile. Edit a server (command, args, env) in one profile: the edit propagates, and if two profiles disagree, the config edited most recently wins (key order alone is never a difference). Remove a server: the removal propagates, but **only with a witness**. `~/.claude/scripts/mcp-ledger.tsv` records which servers *each* config held at the end of the last sync, and a server is removed everywhere only when some config (a) is on record as having held it, (b) does not hold it now, and (c) still holds at least one other server. That is the only state that means "you deleted it here". A profile that has never synced has no ledger rows and can never trigger a removal; a config the app reset to zero servers is excluded by (c). `--no-deletes` skips removals entirely and copies the missing servers back, which is the restore path for an accidental delete.
...
3. **Profile config.** Both blocks of every `claude_desktop_config.json` (`mcpServers` and `preferences`) are reconciled in one read-backup-write pass per file, so a config is never written twice in a run. Runs whether Claude is open or closed.
...
- **Removals need a witness.** A propagated MCP server removal must name the config that lost it (see [Profiles](#profiles-claude-deck)); settings are never deleted by a sync.
